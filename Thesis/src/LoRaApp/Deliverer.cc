...
void Deliverer::initialize(int stage)
{
    ...
    if (stage == INITSTAGE_LOCAL) {
        EV << "Initializing stage 0 at Deliverer line 0\n";
    }
    else if (stage == INITSTAGE_APPLICATION_LAYER) {
        EV << "Initializing stage 0 at Deliverer line 1\n";
        bool isOperational;
...
void Deliverer::handleMessageFromUpperLayer(cMessage *msg)
{
    ...
    if(msg->getKind() == SEND_INVITATION)
    {
    ...
    {
        sendACKrequestSlot(command->getAddress(), command->getSlot(), true);
    }
...
void Deliverer::sendFreeslot_packet(uint8_t slot, double myPRR, simtime_t timeToGW)
{
...
void Deliverer::requestCommunicationSlot(MacAddress destination, uint8_t slot)
{
...
void Deliverer::sendACKrequestSlot(MacAddress destination, int slot, bool accept)
{
...
double Deliverer::getNeighborQuality(MacAddress addr)
{
    ...
    for(int i = 0; i < this->currentNeighbors; i++)
    {
        if(this->neighborsQuality[i].addr == addr)
        {
            for (int j = 0; i < 100; i++)
            {
                if (this->neighborsQuality[i].lastState[j] == true)
                    success++;
            }
        }
    }
