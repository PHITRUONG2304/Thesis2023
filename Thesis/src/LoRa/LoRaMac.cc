...
void LoRaMac::initialize(int stage)
{
    ...
    if (stage == INITSTAGE_LOCAL) {
        EV << "Initializing stage 0: " << stage << endl;
...
//        listening2Time = 1;
        ...
        EV << "My address is: " << this->address << endl;
        ...
        EV << "Initializing stage 1: " << stage << endl;
    }
}

void LoRaMac::finish()
{
    EV_INFO <<"1. My address is: " << address << endl;
...
{
    EV << "process Upper Packet\n";
    Packet *packet = dequeuePacket();
    if(fsm.getState() == IDLE) handleUpperMessage(packet);
}
...
{
    EV <<"handle Can Pull Packet Changed\n";
...
void LoRaMac::handleWithFsm(cMessage *msg)
{
    ...
    FSMA_Switch(fsm)
    {
        FSMA_State(IDLE)
        {
            ...
            EV << "IDLE state" << endl;
            FSMA_Event_Transition(Listening_1-Transmit,
                                  isUpperMessage(msg),
                                  TRANSMIT,
            );
...
{
    EV << "ReceiveSignal: source= " << source << ", signalID= " << signalID << ", value= " << value << ", details=" << details << endl;
    Enter_Method_Silent();
...
Packet *LoRaMac::encapsulate(Packet *msg)
{
    ...
    frame->setReceiverAddress(tag->getDestination());// TODO: Change to this node address
...
Packet *LoRaMac::decapsulate(Packet *frame)
{
    ...
    EV << "--------------------------------------------------" << endl;
    EV <<"TEST transmitter address: " << loraHeader->getTransmitterAddress() << endl;
    frame->addTagIfAbsent<MacAddressInd>()->setSrcAddress(loraHeader->getTransmitterAddress());
    EV <<"TEST receiver address: " << loraHeader->getReceiverAddress() << endl;
    frame->addTagIfAbsent<MacAddressInd>()->setDestAddress(loraHeader->getReceiverAddress());
    EV <<"TEST InterfaceId address: " << networkInterface->getInterfaceId() << endl;
...
void LoRaMac::sendAckFrame()
{
    ...
    auto tag = frameToAck->getTag<LoRaTag>();

    auto macHeader = makeShared<LoRaMacFrame>();
    EV << "Check hello world " <<frameToAck->peekAtFront<LoRaMacFrame>()->getTransmitterAddress()<< endl;
    macHeader->setReceiverAddress(MacAddress(frameToAck->peekAtFront<LoRaMacFrame>()->getTransmitterAddress().getInt()));
    macHeader->setTransmitterAddress(getAddress());
    EV << "Transmitter address: " << macHeader->getTransmitterAddress() << endl;
    EV << "Receiver address: " << macHeader->getReceiverAddress() << endl;
    ...
    frame->addTagIfAbsent<PacketProtocolTag>()->setProtocol(&Protocol::apskPhy);

    auto loraTag = frame->addTagIfAbsent<LoRaTag>();


    loraTag->setBandwidth(tag->getBandwidth());
    loraTag->setCenterFrequency(tag->getCenterFrequency());
    loraTag->setSpreadFactor(tag->getSpreadFactor());
    loraTag->setCodeRendundance(tag->getCodeRendundance());
    loraTag->setPower(tag->getPower());

    auto ackPkt = encapsulate(frame);
...
{
    return frame->getReceiverAddress() == address || frame->getReceiverAddress() == MacAddress::BROADCAST_ADDRESS;
}

//bool LoRaMac::isTimeToSend(const Ptr<const LoRaMacFrame> &frame)
//{
//    return frame->getReceiverAddress() == address || frame->getReceiverAddress() == MacAddress::BROADCAST_ADDRESS;
//}
...
void LoRaMac::handleStartOperation(LifecycleOperation *operation){
    radio->setRadioMode(IRadio::RADIO_MODE_RECEIVER);
}
